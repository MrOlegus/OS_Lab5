...
NTSTATUS OnRegistryNotify(PVOID context, PVOID arg1, PVOID arg2) {
	...
	if (!globals.isLoggingActive) {
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "Loggin disabled");

		return STATUS_SUCCESS;
	}

	ULONG pid = HandleToULong(PsGetCurrentProcessId());
	if ((pid != globals.targetPID)) {
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "Uninteresting process, id: %u", pid);
		return STATUS_SUCCESS;
	}
	...
	if (!NT_SUCCESS(status)) {
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "Unable to open file");
		return STATUS_SUCCESS;
	}

	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "I CREATED FILE!");
	...
	CmCallbackGetKeyObjectIDEx(&globals.regCookie, info->Object, NULL, &name, 0);
	...
	if (!NT_SUCCESS(status)) {
		DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "Unable to write file");
	...
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "Changes logged");
